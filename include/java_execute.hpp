...
class JavaExecute : public ExecuteInterface
{
private:
...
public:
    JavaExecute(wsp::workspace *spc, json &taskData) : spc_ptr(spc),
                                                       taskData(taskData),
                                                       task(taskData["task"])
    {
        taskID = task["id"];
    ...
    void execute() override
    {
        ...
        for (int i = 1; i <= testCount; i++)
        {
            ...
            { // 子进程
                ...
                // 执行
                execvp("java", const_cast<char *const *>(args.data()));
